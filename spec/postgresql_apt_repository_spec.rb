...
describe 'database_sl::postgresql_apt_repository' do
  ...
  it 'installs all packages when specifying db server' do
    valid_attributes = {
      package_name: %w(postgresql postgresql-contrib),
      version: repository_version
    }
    expect(chef_run).to install_package('specifying db server').with(valid_attributes)
  end
  ...
  it 'installs all packages when specifying db client' do
    ...
    expect(chef_run).to install_package('specifying db client').with(valid_attributes)
  end
